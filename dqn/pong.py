...
class DQNAgent:
    ...
    def __init__(self, env, device):
        """초기화."""
        self.net = DQN(env.observation_space.shape, ACTION_SIZE)\
            .to(device)
        # self.net.apply(self.weights_init)
        self.target_net = DQN(env.observation_space.shape, ACTION_SIZE)\
            .to(device)
    ...
    def train_code1(self):
        ...
        # numpy -> torch FloatTensor로 변환
        histories = torch.from_numpy(histories).float().to(device)
        next_histories = torch.from_numpy(next_histories).float().to(device)
        ...
        targets = Variable(torch.from_numpy(targets)).float().to(device)
